...
def linear_conc_operator( c, i ):
    ...
    return (c[i] * (1 - 2 * r + b * g(i)**2) + c[i + 1] * r * (1 - s * g(i)) + c[i - 1] * r * (1 + s * g(i)))
...
def main( name ):
    ...
    c = list(map( start, linspace( 0, thickness, nodes ) ))
    c[0] = c_out
    c[nodes-1] = c_in
    tau = 0
    ...
    plt.ylabel("Концентрация иона \\( \\mathrm{Na}^+ \\), моль/\\(\\text{м}^3\\)")
